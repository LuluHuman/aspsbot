exports.run = async (client, message, args, { MessageEmbed }, admin) => {
	try {
		...
		if (!target) { message.channel.send(error("Provide a person to mute")); return; };
		...
		gm.timeout(604800000, (args[1] ? args[1] : "nothing"))
			.then(() => {
				const embed = success("User has been <:Benched:868094337760759818> (muted) for " + (args[1] ? args[1] : "nothing"))
		...
		require("./../src/log.js")(message.author.username,"Muted "+ target.username+ " for " +(args[1] ? args[1]:"nothing"))
